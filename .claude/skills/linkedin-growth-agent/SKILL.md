...
└── linkedin-posts/                         ← LinkedIn channel folder (this skill writes here)
    ...
    └── posts/                              ← one folder per generated post
        └── <slug>-<YYYYMMDD>/
            ...
            ├── impact-1.svg                    ← text/image posts: two variations
            ├── impact-2.svg
            ├── deck.py                         ← CAROUSELS ONLY: builds every slide
            ├── slide-1.svg .. slide-N.svg      ← generated by deck.py, never hand-authored
            ├── exports/
            │   ├── impact-1.png                ← auto-converted by image-gen-agent
            │   ├── impact-2.png
            │   ├── slide-1.png .. slide-N.png  ← carousels: review renders
            │   └── <slug>-carousel.pdf         ← carousels: THE upload artifact
...
4. **Carousel only for genuine frameworks** with a diagnostic or checklist worth saving, or for an explainer whose mechanism is the point. Put the saveable slide at **position 2, not the end** — if people don't swipe, a slide-7 payoff is never seen. Then follow the **Carousel track** below: a carousel is a caption AND a deck, with different jobs.
...
### Carousel track (when Format = carousel)

A carousel is **two deliverables, not one**: a caption and a deck. Getting the division of labour wrong is the single most common failure, and it fails in both directions. A caption that teases ("swipe for the details") wastes the ~90% who never swipe. A caption that narrates the deck makes the deck pointless.

**The contract:**

| | Job | Test it by asking |
|---|---|---|
| **Caption** | Deliver the **complete argument** in prose. Simple, clear, standalone. | If this reader never swipes, do they still leave with the decision? |
| **Deck** | Deliver the **proof and the mechanism**. Detail, transcripts, charts, lists. | Does a reader who already read the caption still learn something specific? |

**The overlap rule.** Only the load-bearing facts appear in both: the two or three numbers and the one reframe the argument rests on. Everything *shown* rather than *stated* stays exclusive to the deck: transcripts, charts, tier breakdowns, block lists, the closing question. State the overlap explicitly in `01-post-plan.md` so it is a decision, not an accident.

**Narrative arc for an explainer deck.** Order carries more of the work than any individual slide:

1. **The pain, felt.** Recognition, not explanation. The reader should see their own week in it before they read a word of argument. No statistics here.
2. **The pain, measured.** Now the number. A statistic lands far harder once the reader has already admitted the behaviour it measures. Leading with it turns slide 1 into a claim they can argue with instead of a mirror.
3. **The shift.** What actually changed, stated plainly.
4. **The map**, when alternatives exist. Where this sits against the other options, and what each one costs. Answers "why this" before the deck explains "how this."
5. **The mechanism.** How it works, in as few slides as the idea genuinely needs.
6. **What it catches, then what it misses.** Two slides, never merged. See below.
7. **The reader's decision.** Not a summary. The rule that falls out of the miss.

**Publish the unflattering number.** The limitation slide is not a disclaimer to soften, it is what converts an endorsement into a decision framework. A precise weakness yields a precise rule, and the rule is the payoff of the whole deck. Where a source publishes its own failure rate, quote it and name that it came from them.

**Slide craft:**

- **Put the saveable slide at position 2**, per the format rules above. In an explainer deck that is usually the measured pain.
- One idea per slide. If a slide needs two headlines, it is two slides.
- Number every slide `n / N` so the reader knows the depth they are committing to.
- Build the deck with a **`deck.py` script** in the post folder, never hand-authored SVGs. Decks get reordered and renumbered repeatedly; a script makes that a two-line change instead of a rewrite. Reuse the palette, `frame()`, `eyebrow()`, `footer()` and block helpers from the most recent deck so every carousel on the account reads as one system.
- Set commands, paths and transcripts in **mono**; prose in sans.
- **Render and look at every slide before declaring done.** Text overflow and block collisions are invisible in the source and obvious in the PNG. Check the right margin and the gap above the footer on each one.

**Build and export:**

```bash
OUT_DIR=. python3 deck.py
for i in $(seq 1 N); do rsvg-convert -w 1080 -h 1350 slide-$i.svg -o exports/slide-$i.png; done
rsvg-convert -f pdf -o exports/<slug>-carousel.pdf $(for i in $(seq 1 N); do echo slide-$i.svg; done)
```

The **PDF is the upload artifact** for a LinkedIn document post. The PNGs are for review and for reuse as single images.

**Editor scores a carousel on** the standard rubric plus three carousel-specific checks: does the caption stand alone, does the deck avoid restating the caption, and does slide 1 earn the swipe without explaining anything.
...
**Carousels skip this agent.** When Format = carousel, the deck IS the image work: build `deck.py` per the Carousel track above and render the slides and PDF. Do not also generate `impact-*.svg`, unless a single-image fallback is wanted for a text-only posting day, in which case note it in `final-post.md` as an alternative.
...
### Carousel variant of the template

When Format = carousel, replace the `## Final Post` and `## Images` sections with these. Everything else is unchanged.

```markdown
**Format:** <N>-slide document carousel + caption.

## Caption (carousel)

<one line stating what the caption is doing and what it deliberately leaves to the deck>

<paste-ready caption — the complete argument, standalone>
...
**Exports:** `exports/slide-1.png` .. `exports/slide-<N>.png` · **Upload this:** `exports/<slug>-carousel.pdf`

<a short note on WHY the deck is ordered this way — the arc, and anything deliberately held back from an earlier slide>
```
