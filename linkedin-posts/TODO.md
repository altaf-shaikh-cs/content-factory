...
**Gate count: 4** (5 minus token-usage-spike, published 2026-09-10). The gate still fires at ≥3.
...
## Done
- [x] token-usage-spike.md → [posts/token-usage-spike-20260905/final-post.md](./posts/token-usage-spike-20260905/final-post.md) — **published 2026-09-10** as a native video post (`explainer-hook3-light-v2.mp4`, 1080x1350, light, 45.6s), caption unchanged from the 9/5 mechanism rewrite. Confirmed published by Altaf, so the 9/5 attribution ambiguity below no longer blocks this row. Video went through three revisions: a title-card opening replacing the cold numbers, the light theme, and a new "The brake" scene explaining the 200k/160k auto-compact ceiling and what switching on 1M removes.
